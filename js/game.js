...
function jalankanRonde() {
    ...
    // Pastikan Panel Terkunci Setiap Kali Memulai Ronde (Mencegah bug saat klik "Lewati")
    ...
    // KONTROL SELIMUT HITAM DARI CSS CLASS
...
function setupGame2() {
    ...
    let opsiBenar = { 
        id: targetGameData.id,
        title: targetGameData.title, // Disimpan untuk ditampilkan saat evaluasi jika salah
        image: targetGameData.imageFilename, 
        benar: true 
    };

    // 2. Cari 3 Data Pengecoh (Distractor) yang Memiliki Gambar
    // Ambil semua data valid, saring yang punya gambar, dan bukan target
    let allValidPhotoRecords = Object.values(Records).filter(r => r.lat && r.lon && r.imageFilename && r.id !== targetGameData.id);
    
    // Acak dan ambil 3
    let distractors = allValidPhotoRecords
    ...
    let options = [opsiBenar, ...distractors];
...
function bukaPanelEksklusif(qid) {
    ...
    // BUKA KUNCI KLIK PANEL (Class dihapus sementara)
    const panelMobile = document.getElementById('panel');
    if (panelMobile) {
...
function tutupPanelEksklusif() {
    ...
    // KUNCI KEMBALI PANEL UNTUK RONDE BERIKUTNYA
    const panelMobile = document.getElementById('panel');
    if (panelMobile) {
